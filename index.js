...
function filterFundedOnly() {
  ...
  let fullyFundedGames = GAMES_JSON.filter((games) => {
    return games.pledged >= games.goal;
  });
...
let unfundedCount = GAMES_JSON.filter((games) => {
  return games.pledged < games.goal;
}).length;

// create a string that explains the number of unfunded games using the ternary operator
const description = `A total of $${totalRaised.toLocaleString()} has been raised for ${
  GAMES_JSON.length
} games! 
    ${
      unfundedCount > 1
    ...
    }. We need your your help to find these amazing games!`;
if (unfundedCount == 0) {
...
const [fistGame, secondGame] = sortedGames;

// create a new element to hold the name of the top pledge game, then append it to the correct element
const firstGameName = document.createElement("h2");
firstGameName.textContent = fistGame.name;
